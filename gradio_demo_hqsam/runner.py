...
from gradio_demo_hqsam.oss_ops_inference import main_oss_ops
...
class Runner:
    def __init__(self,
        ...
        elif self.model_source == 'hq':
            self.sam = sam_model_registry_hq[model_type](checkpoint=sam_checkpoint)
        else:
            raise ValueError('model_source should be hq or None')
    ...
    def inference_oss_ops(self, prompt, target1, target2, version):
        ...
        if version == 'version 1 (🔺 multiple instances  🔻 whole, 🔻 part)':
            ...
            self.prompt_mask_g = (prompt['mask'][..., 0] != 0)[None, ...]  # 1, H, w
            self.version = 3
        if self.model_source == 'hq':
            self.pred_masks, self.pred_mask_lists = main_oss_ops_hqsam(
            sam=self.sam,
            dinov2=self.dinov2,
